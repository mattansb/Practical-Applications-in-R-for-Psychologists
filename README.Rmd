...
This Github repo contains all lesson files for the *Practical Applications in R* portion of the graduate-level course: ***Advanced Research Methods for Psychologists***, taught at Ben-Gurion University on the Negev (*Fall semester, 2020*).
...
This course will lay the foundation for the topic-focused courses (*Spring semester*):
...
get_src <- function(pkg) {
  if (is.null(src <- packageDescription(pkg)$Repository)) {
    if (is.null(src <- packageDescription(pkg)$GithubRepo)) {
      src <- "Dev"
    }
  }
  return(src)
